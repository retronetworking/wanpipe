...
unsigned char tx_file[WP_LIB_IFNAME_SZ];
unsigned char rx_file[WP_LIB_IFNAME_SZ];
...
int init_args(int argc, char *argv[])
{
	...
	for (i = 1; i < argc; i++){
		...
		if (!strcmp(argv[i],"-i")){
			...
			wanpipe_min_port_no = atoi(argv[i+1]);
			i++;
			continue;
			...
			strncpy((char*)tx_file, argv[i+1],WP_LIB_IFNAME_SZ);
			...
			strncpy((char*)rx_file, argv[i+1],WP_LIB_IFNAME_SZ);
			...
			rbs_events = 1;
		}else if(!strcmp(argv[i],"-dtmf_seq_check")){
			dtmf_seq_check = 1;
...
"\n"
"<options>:\n"
"	-p  <max port number>	#port/span number\n"
"	-pm <min port number>	#port/span number\n"
