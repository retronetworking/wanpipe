...
int MakeConnection (char *i_name ) 
{
	...
	sock_fd[LISTEN].sock = CreateListenSock(i_name);
	if (!sock_fd[LISTEN].sock){
		return -EINVAL;
	}
	...
	for (i=0; i<tx_channels; i++){
		...
		/* Create a new socket */
   		sock_fd[i].sock = socket( AF_WANPIPE, SOCK_RAW, AF_ANNEXG_WANPIPE);
   		if( sock_fd[i].sock < 0 ) {
      			perror("Socket: ");
			sock_fd[i].sock=0;
			return FALSE;
			...
			for (i=0;i<tx_channels;i++){
				...
				if (FD_ISSET(sock_fd[i].sock,&oobset)){
					if (handle_oob_event(i)){
						close_connection(i);
						if (i==LISTEN){
							/* If OOB message is on
							 * listen socket, something
							 * drastic happened, like
							 * RESTART or SHUTDOWN,
							 * thus exit program:
							 * FIXME: handle it ...*/
							return;
...
svc_action:
	...
	for (i=0;i<tx_channels;i++){
		...
		ioctl(sock_fd[i].sock,SIOC_X25_CLEAR_CALL,NULL);
		close(sock_fd[i].sock);
		sock_fd[i].sock=0;
	}
} 
...
int accept_incoming_call(int sock)
{
	...
	if (sock1 < 0){
		printf ("Sock Accept Call FAILED BAD,no sock %i\n",sock1);
		return -EINVAL;
	}	

	if (index >= MAX_SOCK_NUM){
		close(sock1);
		printf("Maximum incoming SVCs reached!\n");
		return -EINVAL;
	}
	
	if ((err=ioctl(sock1,SIOC_X25_GET_CALL_DATA,&api_cmd)) != 0){
		printf("Failed to Get Call data %i\n",err);
		close(sock1);
		return -EINVAL;
	}
	...
	sock_fd[index].mru=api_cmd.hdr.mru;
	sock_fd[index].mtu=api_cmd.hdr.mtu;
	...
	if ((err=ioctl(sock1,SIOC_X25_SET_LCN_PID,&pid))){
		perror("Set LCN PID: ");
	...
	if ((err=ioctl(sock1,SIOC_X25_SET_LCN_LABEL,prognamed))){
		perror("Label: ");
	...
	printf("%s: Rx Call Accept OK %i\n\n",
			prognamed,api_cmd.hdr.lcn);
