...
#		if ( ! -e "/$wanpipe_conf_dir/scripts/start" & $is_trixbox==$FALSE && $is_smg==$FALSE ){ 
		if ($num_zaptel_config != 0){	
...
#			print ("\nWould you like to execute \'ztcfg\' each time wanrouter starts?\n");
#			if (&prompt_user_list("YES","NO","") eq 'YES'){
#				exec_command("cp -f $wanpipe_conf_dir/samples/wanpipe_zaptel_start $wanpipe_conf_dir/scripts/start");	
#			}	 
		}
		if ($is_trixbox==$TRUE){
			print "\nStarting Amportal...\n";
...
sub get_bri_group{
	...
	while(length($res_group)==0 |!($res_group =~/(\d+)/)| $res_group eq '0'){
		...
		$res_group=&prompt_user("Input the group for this port",$def_bri_group);
	}
	$group=$res_group;
	$def_bri_group=$group;
	return $def_bri_group;
}
...
select_bri_option:
					...
					if($def_bri_option eq 'YES'){
						$skip_card=$FALSE;
						$bri_conf.="\n;Sangoma A$1 port $5 [slot:$3 bus:$4 span:$current_tdmapi_span]";
				...
				if ( $a10x->signalling eq 'PRI CPE' | $a10x->signalling eq 'PRI NET' ){    
					if ($silent==$FALSE && $config_zapata==$TRUE){
						printf ("Select switchtype for AFT-A%s on port %s \n", $card->card_model, $port);
							...
							$a10x->pri_switchtype($silent_switchtype);
						}
					}
