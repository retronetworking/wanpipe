...
static int 	xilinx_devel_ioctl(sdla_t *card,struct ifreq *ifr);
static int 	xilinx_write_bios(sdla_t *card, wan_cmd_api_t*);
static int 	xilinx_write(sdla_t *card, wan_cmd_api_t*);
static int 	xilinx_read(sdla_t *card, wan_cmd_api_t*);
...
static int new_if (wan_device_t* wandev, struct net_device* dev, wanif_conf_t* conf)
{
	...
	if (!chan->dma_mtu){
		DEBUG_EVENT("%s:%s: Error invalid MTU %i  mru %i\n",
			card->devname,
			chan->if_name,
			card->wandev.mtu,card->u.xilinx.cfg.mru);
		err= -EINVAL;
...
static int if_send (struct sk_buff* skb, struct net_device* dev)
{
	...
	if (chan->common.state != WAN_CONNECTED){
		...
		if (WAN_NETIF_CARRIER_OK(dev)){
			DEBUG_EVENT("%s: Critical Error: Carrier on on tx dev down\n",
					chan->if_name);
		}
		...
		DEBUG_EVENT("%s: Critical Error: Tx Pending List Full\n",
					chan->if_name);
		WAN_NETIF_STOP_QUEUE(dev);
		return 1;
		...
		err=xilinx_dma_te3_tx(card,chan,skb);	
...
static int if_do_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
	...
	switch(cmd)
	{
		...
		case SIOC_WAN_DEVEL_IOCTL:
			err = xilinx_devel_ioctl(card, ifr);
			break;
...
static int xilinx_init_tx_dev_fifo(sdla_t *card, private_area_t *chan, unsigned char wait)
{
	...
	cur_dma_ptr=get_current_tx_dma_ptr(reg);
        ...
        /* Clean TX DMA fifo */
        dma_descr=(unsigned long)(cur_dma_ptr<<4) + XILINX_TxDMA_DESCRIPTOR_HI;
        reg=0;
        wan_set_bit(INIT_DMA_FIFO_CMD_BIT,&reg);
...
static void xilinx_dma_tx_complete (sdla_t *card, private_area_t *chan, int wtd)
{
	...
	if (wan_skb_queue_len(&chan->wp_tx_pending_list)){
		aft_pending_tx(card,chan);
	}
...
static int fifo_error_interrupt(sdla_t *card, u32 reg, u32 rx_status, u32 tx_status)
{
	u32 err=0;
	u32 i;
	private_area_t *chan;
	...
	if (card->wandev.state != WAN_CONNECTED){
        	DEBUG_EVENT("%s: Warning: Ignoring Error Intr: link disc!\n",
                                  card->devname);
                return 0;
        }
...
//	wp_debug_func_add(__FUNCTION__);
	...
	if (IS_TE3(&card->fe.fe_cfg)){
		...
		DEBUG_EVENT("%s: Internal Error (Never should happened)!\n",
				card->devname);
	}
...
static void wp_aft_te3_isr (sdla_t* card)
{
	...
	if (wan_test_bit(ERROR_INTR_ENABLE_BIT,&reg)){ 
			...
			fifo_error_interrupt(card,reg,rx_fifo_status,tx_fifo_status);
		}
	}
       ...
       /* -----------------2/6/2003 9:37AM------------------
		...
		for (i=0; i<num_of_logic_ch;i++){
			if ((wan_test_bit(i,&dma_rx_reg)|| wan_test_bit(i,&rx_fifo_status)) && 
				...
				chan=(private_area_t*)card->u.xilinx.dev_to_ch_map[i];
				if (!chan){
					DEBUG_EVENT("%s: Error: No Dev for Rx logical ch=%i\n",
							card->devname,i);
					continue;
				}

				if (!wan_test_bit(0,&chan->up)){
					DEBUG_EVENT("%s: Error: Dev not up for Rx logical ch=%i\n",
                                                        card->devname,i);
                                        continue;
				}	
...
isr_skb_rx:
		...
		for (i=0; i<num_of_logic_ch ;i++){
			if (wan_test_bit(i,&dma_tx_reg) && wan_test_bit(i,&card->u.xilinx.logic_ch_map)){
				chan=(private_area_t*)card->u.xilinx.dev_to_ch_map[i];
				if (!chan){
					DEBUG_EVENT("%s: Error: No Dev for Tx logical ch=%i\n",
							card->devname,i);
					continue;
				}

				if (!wan_test_bit(0,&chan->up)){
                                        DEBUG_EVENT("%s: Error: Dev not up for Tx logical ch=%i\n",
                                                        card->devname,i);
                                        continue;
                                }
...
static void handle_front_end_state(void *card_id)
{
	...
	WAN_FECALL(&card->wandev, get_fe_status, (&card->fe, &status));
...
static int xilinx_t3_exar_chip_configure(sdla_t *card)
{
	...
	if (IS_DS3(&card->fe.fe_cfg)){
		...
		DEBUG_EVENT("%s: Error: T3 Exar doesn't support non T3/E3 interface!\n",
				card->devname);
		return -EINVAL;
	}
	...
	xilinx_delay(1);
#if 0
	framer_reset(card);           /* was for A105 proto */
#endif				      /* M.F. remove for A300 */

#if 0
	write_cpld(card,0x00, 0x00);  /* M.F. was 0x01 for A105 proto */
                                      /* M.F.     0x00 for A300 */
                                      /* set DS3 interface mode    */ 
	write_cpld(card,0x01, 0x00);  /* clear TxAIS bit           */

	// 3. Setup Framer
        write_framer(card,0x00, 0x6F); /* DS3 - mode           (bit 6)*/
                                  /* Internal LOS enable  (bit 5)*/
                                  /* Interrupt enabl.reset(bit 3)*/
                                  /* Frame format M13     (bit 2)*/
                                  /* TimRef - Mode 3  (code '1X')*/
        write_framer(card,0x01, 0xA2); /*   */
#else

//FIXME: Alex to put in Exar T3/E3 Configuration
	DEBUG_EVENT("%s: Config %s Front End: Clock=%s\n",
			card->devname,
			(IS_DS3(&card->fe.fe_cfg))?"T3":"E3",
	...
	if (sdla_te3_config(&card->fe, &card->wandev.fe_iface)){
       		DEBUG_EVENT("%s: Failed %s configuratoin!\n",
                               	card->devname,
                             	MEDIA_DECODE(card->fe.fe_cfg.media));
             	return -EINVAL;
       	}
#endif
	for (;;){
...
static void framer_reset(sdla_t *card)
{
        ...
        j  = (unsigned short)read_cpld(card,0x00);
        j  = j & 0x00FE;
        write_cpld(card,0x00, j);
                // delay
                for(i=0;i<10;i++);
        j = j | 0x0001;
        write_cpld(card,0x00, j);
...
#define card	    chan->card
	...
	if (wan_test_bit(TxDMA_HI_DMA_GO_READY_BIT,&reg)){
		DEBUG_EVENT("%s: Error: TxDMA GO Ready bit set on dma (chain=%i) Tx 0x%X\n",
	...
	DEBUG_TEST("%s: TXDMA_LO=0x%X PhyAddr=0x%X DmaDescr=0x%X\n",
...
static int xilinx_dma_te3_tx (sdla_t *card,private_area_t *chan, struct sk_buff *skb)
{
	...
	if (chan->tx_chain_indx >= MAX_AFT_DMA_CHAINS){
		DEBUG_EVENT("%s: MAJOR ERROR: TE3 Tx: Dma tx chain = %i\n",
	...
	DEBUG_TEST("%s: DMA Chain %i:  Cur=%i Pend=%i\n",
			chan->if_name,dma_chain->index,
			chan->tx_chain_indx,chan->tx_pending_chain_indx);

	//FIXME: BIT FAT WARNING: THE index Values
	//are not SMP protected figure it out !!!!!
...
static void aft_pending_tx(sdla_t *card, private_area_t *chan)
{
	...
	while ((skb=wan_skb_dequeue(&chan->wp_tx_pending_list))!=NULL){
		err=xilinx_dma_te3_tx(card,chan,skb);	
		if (err==-EBUSY){
			wan_skb_queue_head(&chan->wp_tx_pending_list,skb);
			break;
		}
	}
}
...
//		__FUNCTION__,reg,bus_addr,dma_descr);
	...
	if (chan->hdlc_eng){
		//reg|=(chan->dma_mtu>>2)&RxDMA_HI_DMA_DATA_LENGTH_MASK;
//NENAD FIXME: USE ABOVE
...
static int xilinx_dma_rx(sdla_t *card, private_area_t *chan, int gcur_ptr)
{
	...
	for (i=0;i<max_dma_cnt;i++){
		...
		if (!dma_chain->skb){
			DEBUG_EVENT("%s: Warning Rx chain = %i: no free rx bufs\n",
					chan->if_name,dma_chain->index);
			wan_clear_bit(0,&dma_chain->init);
			err=-EINVAL;
			break;
		...
		if (err){
			DEBUG_EVENT("%s: Rx dma chain %i overrun error: should never happen!\n",
					chan->if_name,dma_chain->index);
...
static void aft_init_tx_rx_dma_descr(private_area_t *chan)
{
	...
	for (i=0;i<MAX_AFT_DMA_CHAINS;i++){
		...
		tx_dma_descr=(unsigned long)(i<<4) + XILINX_TxDMA_DESCRIPTOR_HI;
		rx_dma_descr=(unsigned long)(i<<4) + XILINX_RxDMA_DESCRIPTOR_HI;
...
#define card	    chan->card
	 ...
	 * all chains are busy */
	if (!wan_test_bit(0,&dma_chain->init)){
		DEBUG_EVENT("%s: (%s) Error: Pending chain %i empty!\n",
			chan->if_name,__FUNCTION__,dma_chain->index);
		return;
	}
	 ...
	 * all are busy */
	if (!wan_test_bit(RxDMA_HI_DMA_GO_READY_BIT,&reg)){
		DEBUG_EVENT("%s: (%s) Error: Pending chain %i Go bit is not set!\n",
			chan->if_name,__FUNCTION__,dma_chain->index);
		return;
	}
...
static void aft_free_rx_descriptors(private_area_t *chan)
{
	...
	chan->rx_pending_chain_indx = chan->rx_chain_indx;
...
static int aft_hdlc_core_ready(sdla_t *card)
{
	...
	if (err){
	DEBUG_EVENT("%s: Critical Error: HDLC Core not ready!\n",
			card->devname);
	}
	return err;
}
