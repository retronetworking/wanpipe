...
* 				off.
* Apr 25, 2001  Nenad Corbic	Fixed the 2.0.X kernel bug in pci_init.
...
//ALEX# include <linux/sdla_front_end.h>
...
static int sdla_bus_write_1(void* phw, unsigned int offset, u8);
static int sdla_bus_write_2(void* phw, unsigned int offset, u16);
static int sdla_bus_write_4(void* phw, unsigned int offset, u32);
static int sdla_bus_read_1(void* phw, unsigned int offset, u8*);
static int sdla_bus_read_2(void* phw, unsigned int offset, u16*);
static int sdla_bus_read_4(void* phw, unsigned int offset, u32*);
...
int sdladrv_init(void* arg)
{
	...
	if (WANPIPE_VERSION_BETA){
		DEBUG_EVENT("%s Beta%s-%s %s %s\n",
			wan_fullname, WANPIPE_SUB_VERSION, WANPIPE_VERSION, 
	...
	if (sdla_memory_map(hw, SDLA_CPU_A) == 0){
		...
		case A101_ADPTR_2TE1:
			...
			case AFT_SECURITY_2LINE_CH:
				...
				break;
			default:
				DEBUG_EVENT("%s: AFT-A101-2 Critical error: Unknown Security ID (0x%02X)!\n",
...
static int sdla_s514_hw_select (sdlahw_card_t* hwcard, int cpu_no, int irq, void* dev)
{
	...
	case S5142_ADPTR_2_CPU_SERIAL:
		if ((hw = sdla_hw_register(hwcard, cpu_no, irq, dev))){
			...
			number_of_cards += 2;
#if defined(__LINUX__)
			if ((hw = sdla_hw_register(hwcard, SDLA_CPU_B, irq, dev))){
				sdla_save_hw_probe(hw, 0);
				sdla_save_hw_probe(hw, 1);
				number_of_cards += 2;
			}
...
static int sdla_aft_hw_select (sdlahw_card_t* hwcard, int cpu_no, int irq, void* dev)
{
	...
	case A101_ADPTR_1TE1:
		...
		if ((hw = sdla_hw_register(hwcard, cpu_no, irq, dev))){
			...
			sdla_save_hw_probe(hw, 0);
			number_of_cards += 1;
			DEBUG_EVENT("%s: %s T1/E1 card found (%s rev.%X), cpu(s) 1, bus #%d, slot #%d, irq #%d\n",
					wan_drvname,
					hwcard->adptr_name,
...
#endif
			DEBUG_EVENT("%s: %s T1/E1 card found (%s rev.%X), cpu(s) 2, bus #%d, slot #%d, irq #%d\n",
					wan_drvname,
               		        	hwcard->adptr_name, 
					AFT_CORE_ID_DECODE(hwcard->core_id),
					hwcard->core_rev,
	...
	case A104_ADPTR_4TE1:
		...
		if ((hw = sdla_hw_register(hwcard, cpu_no, irq, dev))){
			...
			number_of_cards ++;
			DEBUG_EVENT("%s: %s T1/E1 card found (%s rev.%X), cpu(s) 1, line(s) 4, bus #%d, slot #%d, irq #%d\n",
					wan_drvname,
			               	hwcard->adptr_name,
					AFT_CORE_ID_DECODE(hwcard->core_id),
					hwcard->core_rev,
	...
	case A300_ADPTR_U_1TE3:
		...
		if ((hw = sdla_hw_register(hwcard, cpu_no, irq, dev))){
			...
			sdla_save_hw_probe(hw, 0);
			number_of_cards += 1;
			DEBUG_EVENT("%s: %s T3/E3 card found (%s rev.%X), cpu(s) 1, bus #%d, slot #%d, irq #%d\n",
					wan_drvname,
               	        		hwcard->adptr_name,
					AFT_CORE_ID_DECODE(hwcard->core_id),
					hwcard->core_rev,
...
# endif
		...
		case V3_VENDOR_ID:
			if (dev->subvendor_id == SANGOMA_SUBSYS_VENDOR){
				...
				hwcard->core_id	= 
					dev->subsystem_id & AFT_CORE_ID_MASK;
				hwcard->core_rev= 
					(dev->subsystem_id & AFT_CORE_REV_MASK) >> 8;
...
sdla_card_register(unsigned char hw_type, int slot_no, int bus_no, int ioport)
{
	...
	if (new_hwcard){
		if (hw_type == SDLA_PCI_CARD){
			DEBUG_TEST("%s: Card is already exists (slot=%d,bus=%d)!\n",
					__FUNCTION__,
					slot_no, 
					bus_no);
		}else{
			DEBUG_TEST("%s: Card is already exists (ioport=%d)!\n",
					__FUNCTION__,
					ioport);
		}
		return new_hwcard;
...
sdla_hw_register(sdlahw_card_t* hwcard, int cpu_no, int irq, void* dev)
{
	...
	if (new_hw){
		if (hwcard->hw_type == SDLA_PCI_CARD){
			DEBUG_EVENT("%s: CPU is already exists (slot=%d,bus=%d,cpu=%c)!\n",
					__FUNCTION__,
					hwcard->slot_no,
					hwcard->bus_no, 
					SDLA_GET_CPU(cpu_no));
		}else{
			DEBUG_EVENT("%s: CPU is already exists (ioport=%d)!\n",
					__FUNCTION__,
					hwcard->ioport);
		}
		return new_hw;
...
static int sdla_load (void* phw, void* psfm, unsigned len)
{
	...
	if (hw->memory < sfm->info.memsize){
		DEBUG_EVENT("%s: firmware needs %lu bytes of on-board memory!\n",
...
static int sdla_bus_write_1(void* phw, unsigned int offset, u8 value) 
{
	sdlahw_t*	hw = (sdlahw_t*)phw;
...
static int sdla_bus_write_2(void* phw, unsigned int offset, u16 value) 
{
	sdlahw_t*	hw = (sdlahw_t*)phw;
...
static int sdla_bus_write_4(void* phw, unsigned int offset, u32 value) 
{
	sdlahw_t*	hw = (sdlahw_t*)phw;
...
static int sdla_bus_read_1(void* phw, unsigned int offset, u8* value)
{
	sdlahw_t*	hw = (sdlahw_t*)phw;
...
static int sdla_bus_read_2(void* phw, unsigned int offset, u16* value)
{
	sdlahw_t*	hw = (sdlahw_t*)phw;
...
static int sdla_bus_read_4(void* phw, unsigned int offset, u32* value)
{
	sdlahw_t*	hw = (sdlahw_t*)phw;
