/**************************************************************************
 * sdla_codec.c	WANPIPE(tm) Multiprotocol WAN Link Driver. 
...
int wanpipe_codec_convert_u2s(netskb_t *skb, netskb_t *nskb, u16 *pptr, int codec)
{
	int i;
	u16 	*buf;
	u8 	*data=wan_skb_data(skb);
	u16 	power;

	*power=0;

	for (i=0;i<wan_skb_len(skb);i++){
		buf=wan_skb_put(nskb,2);
		buf[0]=u2s[data[i]];	
		*power+=u2s[data[i]&0x7F];		
	}

	power = (u16)(power/wan_skb_len(skb));
	*pptr=power;
