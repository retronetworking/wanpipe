...
int wanpipe_global_cdev_init(void)
{
	...
	if ((err = register_chrdev(WP_CDEV_MAJOR, "wanpipe", &wp_cdev_fops))) {
		DEBUG_EVENT("%s(): Error unable to register device!\n",__FUNCTION__);
		return err;
	}
	...
	if (cdev_add(&wp_cdev_dev, dev, WP_CDEV_MAX_MINORS)) {
		...
		DEBUG_EVENT("%s(): Error cdev_add!\n",__FUNCTION__);
		return -EINVAL;
	}

	wp_cdev_class = class_create(THIS_MODULE, "wanpipe");
	if (IS_ERR(wp_cdev_class)) {
		DEBUG_EVENT("%s(): Error creating class!\n",__FUNCTION__);
...
int wanpipe_cdev_tdm_create(wanpipe_cdev_t *cdev)
{
	...
	if (!wan_test_bit(0,&wandev.init)) {
		DEBUG_EVENT("%s(): Error global device not initialized!\n",__FUNCTION__);
		return -1;
	}
...
int wanpipe_cdev_timer_create(wanpipe_cdev_t *cdev)
{
	...
	if (wandev.timer_dev_cnt >= WP_MAX_TIMER_DEV_CNT) {
		DEBUG_EVENT("%s(): Error timer dev cnt limit %i!\n",__FUNCTION__,wandev.timer_dev_cnt);
		return -1;
	}
...
int wanpipe_cdev_free(wanpipe_cdev_t *cdev)
{
	...
	if (!CPRIV(cdev)) {
		DEBUG_EVENT("%s(): Error cdev priv not initialized!\n",__FUNCTION__); 
		return -ENODEV;
	}
	...
	if (minor < 0 || minor > WP_CDEV_MAX_MINORS) {
		DEBUG_EVENT("%s(): Error MINOR out of range %i!\n",__FUNCTION__,minor);
		return -EINVAL;
	}
	
	if (!wan_test_bit(0,&cdev->init)) {
		DEBUG_EVENT("%s(): Error cdev device not initialized!\n",__FUNCTION__);
		return -ENODEV;
	}
...
static int wanpipe_create_cdev(wanpipe_cdev_t *cdev, int minor, int *counter)
{
	...
	if (!cdev_priv) {
		DEBUG_EVENT("%s(): Error unable to alloc cdev_priv mem!\n",__FUNCTION__);
		return -ENOMEM;
	}
	...
	if (wandev.idx[minor] != NULL) {
		...
		wan_free(cdev_priv);
		/* Busy */
		DEBUG_EVENT("%s(): Error MINOR device busy %i!\n",__FUNCTION__,minor);
		return 1;
	}
...
static int wanpipe_free_cdev(wanpipe_cdev_t *cdev, int minor, int *counter)
{
	...
	if (wandev.idx[minor] != cdev || !cdev_priv) {
		wan_spin_unlock_irq(&wandev.lock,&flags);
		/* Busy */
		DEBUG_EVENT("%s(): Error MINOR device busy 0x%X! cdev=%p  idx=%p cdev_priv=%p\n",
...
static int wp_cdev_open(struct inode *inode, struct file *file)
{
	...
	if (cdev == NULL) {
		wan_spin_unlock_irq(&wandev.lock,&flags);
		/* No Dev */
		DEBUG_EVENT("%s(): Error cdev is null!\n",__FUNCTION__);
		return -ENODEV;
	}
	...
	if (!wan_test_bit(0,&cdev->init)) {
		wan_spin_unlock_irq(&wandev.lock,&flags);
		/* Dev not initialized */
		DEBUG_EVENT("%s(): Error cdev is not initialized!\n",__FUNCTION__);
		return -ENODEV;
	}
...
static int wp_cdev_release(struct inode *inode, struct file *file)
{
	...
	if (minor > WP_CDEV_MAX_MINORS) {
		DEBUG_EVENT("%s(): Error MINOR is out of range %i !\n",__FUNCTION__,minor);
		return -ENODEV;
	}

	if (wandev.idx[minor] != cdev) {
		DEBUG_EVENT("%s(): Error cdev does not match minor index ptr!\n",__FUNCTION__);
		return -ENODEV;
	}
